...
export default {
	...
	methods: {
		getCategories() {
			return new Promise((resolve, reject) => {
				api.getCategoryNames().then(result => {
					...
					.catch(error => {
						store.dispatch('error/setError', { segment:'getCategoriesError',errorMessage:'Nem sikerült a lekérni a kategóriák neveit',error:true});
		...
		getMenuByCategory(category) {
			return new Promise((resolve, reject) => {
				api.getMenuByCategory(category).then(result => {
					...
					.catch(error => {
						store.dispatch('error/setError', { segment:'getCMenuByCategoryError',errorMessage:'Nem sikerült lekérni a katgóriát',error:true});
		...
		...mapActions('error', ['setError', 'clearErrors'])
